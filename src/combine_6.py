...
from kornia.feature import LocalFeatureMatcher, LoFTR, DenseSIFTDescriptor, gftt_response
...
def image_callback(imageL, imageC, imageR):
  ...
  image_tensor = KF.gftt_response(tensorleft)

  #arr_ = np.squeeze(tensorleft_features)
  b = image_tensor.shape[0]
  for i in range(b):
    img = tensor_to_image(image_tensor, i)
    img.save(f"image_{i}.png")  # Save the image
    # img.show() # If you want to show the image instead

  #print(type(arr_))
  #plt.imshow(arr_)
  #plt.show()

  #print(tensorleft_features)
