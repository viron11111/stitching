...
def image_callback(imageL, imageC, imageR):
  ...
  #canny = kornia.filters.Laplacian(kernel_size=[11,11], border_type='circular', normalized=True).cuda()
  #canny = kornia.filters.Sobel(eps=1e-10).cuda()
  #canny = kornia.filters.SpatialGradient().cuda()
  values = [canny(image) for image in imgs]
  #print("rows: ", len(values))
  #print("cols: ", len(values[0]))

  #values = canny(imgs[:])
  #image_magnitude, image_canny = canny(tensorleft)
  #magnitude, edges= canny(imgs)
  ...
  #tensorleft_byte = [values([edges][1])*255 for edges in len(imgs)]
  #tensorleft_byte = values[0][1]*255
  #tensorleft_byte = values[0] * 255
  back_to_image = [K.tensor_to_image(tensor_byte[x]) for x in range(len(imgs))]
  edge_images = back_to_image[0].astype(np.uint8)
  ...
  try:
    #image_pub.publish(br.cv2_to_imgmsg(imageLeft, 'mono8'))
    image_pub.publish(br.cv2_to_imgmsg(edge_images, 'mono8'))
